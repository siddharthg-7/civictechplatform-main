...
const NavDots = ({ scenes, activeScene, onDotClick, onCTA }) => (
  <div className="fixed right-7 top-1/2 -translate-y-1/2 z-[200] hidden md:flex flex-col gap-[14px]">
    {scenes.map((scene, i) => {
      ...
      return (
        ...
        >
          {/* Tooltip */}
          <span
            className="absolute right-8 whitespace-nowrap px-2.5 py-1 rounded-lg text-[9px] font-black tracking-[0.18em] uppercase opacity-0 translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-200 pointer-events-none border border-white/10"
    ...
    >
      <span
        className="absolute right-8 whitespace-nowrap px-2.5 py-1 rounded-lg text-[9px] font-black tracking-[0.18em] uppercase opacity-0 translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-200 pointer-events-none border border-white/10"
        style={{ background: "rgba(0,0,0,0.9)", color: "rgba(255,255,255,0.5)" }}
      >
        Enter Portal
      </span>
      <span
